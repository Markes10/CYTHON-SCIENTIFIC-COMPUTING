...
- **Q: Why use CSR format?** In physical simulations, stiffness matrices are over 99% sparse. Storing only non-zero elements reduces memory consumption from $O(N^2)$ to $O(N + \text{NNZ})$ and optimizes CPU L1/L2 cache locality.\n
